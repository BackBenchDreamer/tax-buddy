...
export function ValidationPanel({ result }: { result: ValidationResult }) {
  ...
  const statusConfig = {
    ok: { icon: ShieldCheck, label: 'Document Verified', color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', stroke: '#10b981' },
      ...
      <div className="flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left gap-2">
          ...
          <p className="text-sm text-slate-400 mt-2">
            {issues.length === 0 
              ? "All data points perfectly match Form 26AS records. Safe to file." 
              : `Found ${issues.length} discrepancies between Form 16 and backend validation records.`}
          </p>
        </div>
